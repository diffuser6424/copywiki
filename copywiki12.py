...
def remove_lines_with_braces(markdown):
    lines = markdown.split("\n")
    lines = [line for line in lines if not line.startswith("{") and not line.startswith("}") and not line.startswith("[[File:") and not line.startswith("| ")]
    return "\n".join(lines)
